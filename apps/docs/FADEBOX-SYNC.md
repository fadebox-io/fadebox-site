...
| --- | --- | --- |
| 2026-08-20 | Personal API keys and key expiry — a human account can now hold keys, and any key can carry a lifetime. Written ahead of the merge; fadebox PR #109, so the counters above are not bumped. Note this branch is cut from master and does not carry the template-starters row (fadebox-site PR #18, open). | `guides/ci-api-keys.md` — retitled and reframed around both kinds of key, with new "Your own keys" and "Expiry" sections (including why CI keys default to no expiry); `guides/access-control.md` — a key is not only a service-account thing. |
