...
- **Last documented fadebox commit:** `997918b` (master, 2026-08-22)
- **Merged PRs reviewed through:** #121 (#66, #89, #99 and #110 were closed unmerged or are
...
| 2026-08-22 | Timestamps now render in the viewer's own time zone and locale rather than the server's UTC wall clock, and the 12/24-hour Appearance setting reaches every timestamp instead of only the audit log. Fadebox PRs #120 and #121, both merged 2026-08-22; counters above moved with them. #121 is dev-tooling only (dev mode now runs in UTC like the image), but it exposed that the UTC-process requirement was never written down for anyone running the jar directly. API-key expiry also changed shape — a remaining span instead of a bare date, which was zone-dependent and implied a deadline the value does not have. | `guides/audit-log.md` — the event list no longer claims a "UTC timestamp", and a new "Times are yours, storage is UTC" section covers the viewer's zone, what picking a day in the filter means, the Appearance setting, and that the API still speaks raw UTC; `guides/ci-api-keys.md` — Expiry now says a lifetime is a span rather than a calendar date, and what the list shows; `reference/configuration.md` — General gains the UTC-process requirement and why a server on another zone is wrong twice over. |
